...
5. *Links*: This is key thing to get right. It's critical to understand the
   ...
   2. You'd like your local links to point to the correct local spot. You can do
      ...
      (just not =file:/= absolute path).
   *Here's the rules:*
    
   Links for relative static pages to pages and posts. Ensure that there's no
   =/= immediately following =file:=. For example:
   #+BEGIN_EXAMPLE
   ...
   /Important: you must use the =YYYY/MM/DD/blog-title/index.html= format!/
   #+BEGIN_EXAMPLE
