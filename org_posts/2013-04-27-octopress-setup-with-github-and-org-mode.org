...
5. Links to other pages or posts require a relative file path. 
   ...
   format as this example shows. It's easiest to copy the =/blog/...= part from
   ...
   #+END_EXAMPLE
   Note, I had first tried to use =http:= rather than =file:=, and using a the =html= file suffix
